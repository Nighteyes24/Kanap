...
submitOrder.addEventListener("submit", function (event) {
  ...
  let contact = {
    firstName: prenomVerif(),
    lastName: nomVerif(),
    address: adresseVerif(),
    city: villeVerif(),
    email: emailVerif(),
  };
  // Vérification des éléments de contact et lancement de la procédure d'envoi au back end
  if (
    contact.firstName &&
    contact.lastName &&
    contact.address &&
    ...
    const form = {
      contact,
      products: getCart().map((product) => product.id),
    };
    sendOrder(form);
